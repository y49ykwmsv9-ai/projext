...
def main():
 ...
 c.execute("INSERT OR REPLACE INTO sources VALUES(?,?,?,?,?)",("historix-jurisdiction-review","HISTORIX jurisdiction assertion review queue","curated-review","repository://data/historix-master/jurisdiction_assertions.json",json.dumps({"rule":"unresolved records receive no inferred parent"})))
 ...
 for p_id,attrs,parent in c.execute("select place_id,attributes,parent_place_id from places where parent_place_id is not null").fetchall():
  ...
  if a.get("dataset")=="Natural Earth":
   c.execute("insert or ignore into place_jurisdiction(place_id,parent_place_id,jurisdiction_basis,relationship,source_id,source_record_key,confidence,status,claim_status,valid_from,valid_to,notes) values(?,?,?,?,?,?,?,?,?,?,?,?)",
             (p_id,parent,"de_facto","administrative-parent","natural-earth-pinned",a.get("adm1_code") or a.get("adm0_a3"),"source-defined","asserted","administered",None,None,"Natural Earth administrative geography; no de jure sovereignty inferred"))
   de_facto+=1
