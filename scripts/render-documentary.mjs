...
for (const c of manifest.chapters) {
  ...
  for (let i = 0; i < scenes.length; i++) {
    ...
    if (isMapScene) {
      ...
      run('ffmpeg', ['-y', '-loop', '1', '-i', imagePath, '-i', wav, '-f', 'lavfi', '-i', ambience, '-t', String(sec), '-vf', filter, '-filter_complex', '[1:a]aresample=22050,apad=pad_dur=90[n];[2:a]volume=0.16[a];[n][a]amix=inputs=2:duration=first:dropout_transition=2[aout]', '-map', '0:v', '-map', '[aout]', '-r', '24', '-c:v', 'libx264', '-preset', 'veryfast', '-tune', 'stillimage', '-b:v', '850k', '-maxrate', '1000k', '-bufsize', '1700k', '-pix_fmt', 'yuv420p', '-c:a', 'aac', '-b:a', '96k', '-movflags', '+faststart', mp4]);
    }
    sceneFiles.push(mp4);
  }
}
