...
const tabs=['Overview','Politics','Economy','Diplomacy','Military','Intelligence','Production','Technology','Population','Territory','News','History','Commands'];
...
export default function GameShell(){
 ...
 const [selected,setSelected]=useState<NationState|null>(null),[selectedRegion,setSelectedRegion]=useState<string|null>(null);
 ...
 const nation=liveNation?({...makeNation(liveNation.id,liveNation.name),...liveNation} as NationState):undefined;
 ...
 function detail(){
  ...
  if(tab==='News')return <><h3>World News & Intelligence Feed</h3><div className='row'><span>Reports generated</span><b>{worldState.news.length}</b></div><div className='news-list'>{worldState.news.slice(0,40).map(n=><div className={'news-item importance-'+n.importance} key={n.id}><div><b>{n.title}</b><small>{n.date} · {n.category.toUpperCase()} · {n.source}</small></div><p>{n.summary}</p></div>)}</div></>;
