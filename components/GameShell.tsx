...
import {addStrategicRegions} from '../lib/region-data';
...
function seedWorld(date=START_DATE,playerNation?:string):WorldState{
 ...
 for(const f of countries){
  const id=String(f.id??'0'), mapName=f.properties?.name??'Unknown', db=getWorldProfile(id,mapName), n=makeNation(id,db?.name??mapName);
...
export default function GameShell(){
 ...
 function quick(c:string){const s=structuredClone(worldState);const r=issueCommand(s,c,s.playerNation??'');setWorldState(s);setLog(x=>[(r.ok?'✓ ':'✕ ')+r.message,...x].slice(0,12));setShowMenu(true)}
 ...
 return <main className='game-shell'>
  ...
  <div className='floating map-hint'>Pinch / scroll to zoom · drag to pan · tap a country · tabs open the live command panel</div><div className='floating map-modes'><span>MAP</span>{(['political','economy','military','resources'] as const).map(m=><button className={mapMode===m?'active':''} onClick={()=>setMapMode(m)} key={m}>{m}</button>)}</div>
