...
export interface MapEntity{
 ...
 areaKm2:number; population:number; mapSource:string; sourceCode?:string; centroid:[number,number]; geometryKey:string; children:EntityId[]; adjacency:EntityId[]; ratios:RegionRatios; development:number; infrastructure:number; historicalPolityIds?:string[]; historicalInfluence?:number; historicalContext?:string[];
}
...
export interface EventState{id:EntityId;date:DateKey;title:string;description:string;severity:number;options:string[];resolved:boolean;effects?:EventEffect[];historicalOnly?:boolean;trigger?:EventTrigger;category?:'political'|'diplomatic'|'economic'|'military'|'social'|'historical';importance?:number;source?:'player'|'ai'|'system'|'historical';expires?:DateKey;historicalRecordId?:string;historicalLocation?:{lat:number;lon:number};}
export interface NewsArticle{headline:string;dateline:string;lead:string;paragraphs:string[];}
export interface NewsItem{id:EntityId;date:DateKey;title:string;summary:string;category:'political'|'diplomatic'|'economic'|'military'|'social'|'historical';importance:number;relatedNation?:EntityId;relatedEvent?:EntityId;mentionedNations?:EntityId[];read?:boolean;source:'player'|'ai'|'system'|'historical';article?:NewsArticle;}
export interface CausalCatalyst{ id:string;kind:'command'|'war'|'diplomacy'|'economy'|'military'|'social'|'territory';source:string;sourceNation?:EntityId;targetNation?:EntityId;strength:number;expires:number;tags:string[];createdDate:DateKey;causalDepth:number;parentCatalystId?:string; }
