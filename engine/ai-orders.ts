...
export function interpretPlayerInstruction(state:WorldState,raw:string,player:string):AIOrderResult{
 ...
 if(is('recruit','conscription','raise an army','raise troops','expand the army','expand army')){const amount=Math.max(1000,Math.round(numberFrom(q,5000)));change('recruited '+Math.round(amount).toLocaleString()+' personnel',()=>{const id=n.id+'-ai-army-'+state.tick+'-'+Object.keys(state.units).length;state.units[id]={id,nation:n.id,name:'AI-raised Formation',kind:'infantry',strength:amount,organization:55,equipment:65,experience:0,manpower:amount,territory:n.id};n.manpower=Math.max(0,n.manpower-amount/1e6);});}
 if(is('build','construct','expand','industrialize')&&is('factory','factories','industry','industrial','manufactur'))change('industrial capacity expanded',()=>{n.industrialCapacity+=Math.max(1,Math.round(numberFrom(q,1)));n.treasury-=Math.min(n.treasury,Math.max(1,Math.round(numberFrom(q,5))));});
